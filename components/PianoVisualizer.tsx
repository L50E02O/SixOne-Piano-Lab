...
/**
 * Visualizador de piano: muestra las teclas y las letras del teclado PC.
 * Estilo oscuro tipo dashboard de produccion musical.
 * Orden de notas: C3 a C5.
 */
export function PianoVisualizer({ activeKeys = new Set() }: PianoVisualizerProps) {
  return (
    <div className="w-full max-w-5xl mx-auto select-none">
      <div className="flex rounded-lg overflow-hidden shadow-2xl border border-piano-border bg-piano-black">
        {NOTE_ORDER.map((key) => {
          ...
          return (
            <div
              ...
              className={`
                ...
                ${isBlack ? "h-28 -mx-1 z-10 w-6 shrink-0" : "h-48"}
                ${isBlack
                  ? isActive
                    ? "bg-piano-active"
                    : "bg-gray-900"
            ...
            >
              ...
              <span
                className={`text-[10px] mt-0.5 ${isBlack ? "text-piano-white/70" : "text-piano-black/70"}`}
              >
                {info.note}
              </span>
            </div>
